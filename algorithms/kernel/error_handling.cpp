...
ErrorDetailCollection  errorDetailCollection  = ErrorDetailCollection();

template<typename T>
void describeDetailCollection(const Collection<SharedPtr<ErrorDetail<T> > > &details, char *description)
{
    if(details.size() > 0)
    {
...
char *KernelErrorCollection::getDescription()
{
    if(size() == 0)
    {
    ...
    for(size_t i = 0; i < size(); i++)
    {
        ...
        if(e->intDetails().size() > 0 || e->doubleDetails().size() > 0 || e->stringDetails().size() > 0)
        {
            const char *details = "Details:\n";
            cat(details, errorDescription[i]);

            describeDetailCollection<int>(e->intDetails(), errorDescription[i]);
            describeDetailCollection<double>(e->doubleDetails(), errorDescription[i]);

            describeDetailCollection<String>(e->stringDetails(), errorDescription[i]);
        }
...
const char *Error::description() { return errorMessageCollection.find(_id)->description(); }
