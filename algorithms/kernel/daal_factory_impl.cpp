...
Factory::Factory()
{
    ...
    registerObject(new Creator<DataSourceDictionary>());
...
/*
    ...
    registerObject(new Creator<algorithms::optimization_solver::sgd::Result>());
...
void NumericTable::allocateBasicStatistics()
{
    ...
    if (basicStatistics.get(NumericTable::minimum).get() == NULL || basicStatistics.get(NumericTable::minimum)->getNumberOfColumns() != ncols)
    {
        basicStatistics.set(NumericTable::minimum,
                            services::SharedPtr<NumericTable>(new HomogenNumericTable<double>(ncols, 1, NumericTableIface::doAllocate)));
    }
