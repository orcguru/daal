...
-Zl    := $(-Zl.$(COMPILER))
-DEBC  := $(if $(REQDBG),$(if $(OS_is_win),-debug:all -Z7,-g))
...
# OS X release structure (under __release_mac):
# daal
# daal/bin - platform independent binaries: env setters
# daal/examples - usage demonstrations
# daal/include - header files
...
VMLDIR := $(DIR)/externals/mkl/$(_OS)
VMLDIR.include := $(VMLDIR)/include $(VMLDIR)/..
VMLDIR.libia   := $(VMLDIR)/lib/$(_IA)
releasemkl.LIBS_A := $(if $(OS_is_win),$(VMLDIR.libia)/$(plib)daal_mkl_thread.$(a) $(VMLDIR.libia)/$(plib)daal_mkl_sequential.$(a),)
releasemkl.LIBS_Y := $(VMLDIR.libia)/$(if $(OS_is_mac),../,)$(plib)daal_mkl_thread.$(y) $(VMLDIR.libia)/$(if $(OS_is_mac),../,)$(plib)daal_mkl_sequential.$(y)

TBBDIR := $(DIR)/externals/tbb/$(_OS)
...
releasetbb.LIBS_A := $(if $(OS_is_win),$(TBBDIR.libia)/tbb.$(a),)
releasetbb.LIBS_Y := $(TBBDIR.libia)/$(plib)tbb.$(y) $(if $(OS_is_lnx),$(TBBDIR.libia)/$(plib)tbb.so.2,)

IPPDIR := $(DIR)/externals/ipp/$(_OS)
IPPDIR.include := $(IPPDIR)/include $(IPPDIR)/../include
IPPDIR.libia   := $(IPPDIR)/lib/$(_IA)
...
daaldep.lnx32.mkl.seq := -L$(VMLDIR.libia) -ldaal_mkl_sequential
daaldep.lnx32.mkl := $(VMLDIR.libia)/$(plib)mkl_daal_core.$a
daaldep.lnx32.vml := $(VMLDIR.libia)/$(plib)vml_daal.$a
daaldep.lnx32.ipp := $(IPPDIR.libia)/libippdaal.$a $(IPPDIR.libia)/libdaalzlib.$a $(IPPDIR.libia)/libdaalbzlib.$a
daaldep.lnx32.rt  := -L$(TBBDIR.libia) -ltbb -lpthread $(daaldep.lnx32.rt.$(COMPILER))

daaldep.win32e.mkl.thr := $(VMLDIR.libia)/daal_mkl_thread.$a
daaldep.win32e.mkl.seq := $(VMLDIR.libia)/daal_mkl_sequential.$a
daaldep.win32e.mkl := $(VMLDIR.libia)/$(plib)mkl_daal_core.$a
daaldep.win32e.vml := $(VMLDIR.libia)/$(plib)vml_daal.$a
daaldep.win32e.ipp := $(IPPDIR.libia)/ippdaalmt.$a $(IPPDIR.libia)/daalzlib.$a $(IPPDIR.libia)/daalbzlib.$a
daaldep.win32e.rt  := -LIBPATH:$(TBBDIR.libia) tbb.lib libcpmt.lib libcmt.lib

daaldep.win32.mkl.thr := $(VMLDIR.libia)/daal_mkl_thread.$a
daaldep.win32.mkl.seq := $(VMLDIR.libia)/daal_mkl_sequential.$a
daaldep.win32.mkl := $(VMLDIR.libia)/$(plib)mkl_daal_core.$a
daaldep.win32.vml := $(VMLDIR.libia)/$(plib)vml_daal.$a
daaldep.win32.ipp := $(IPPDIR.libia)/ippdaalmt.$a $(IPPDIR.libia)/daalzlib.$a $(IPPDIR.libia)/daalbzlib.$a
daaldep.win32.rt  := -LIBPATH:$(TBBDIR.libia) tbb.lib libcpmt.lib libcmt.lib

daaldep.mac32e.mkl.thr := -L$(VMLDIR.libia)/.. -ldaal_mkl_thread
daaldep.mac32e.mkl.seq := -L$(VMLDIR.libia)/.. -ldaal_mkl_sequential
daaldep.mac32e.mkl := $(VMLDIR.libia)/$(plib)mkl_daal_core.$a
daaldep.mac32e.vml := $(VMLDIR.libia)/$(plib)vml_daal.$a
daaldep.mac32e.ipp := $(IPPDIR.libia)/libippdaal.$a $(IPPDIR.libia)/libdaalzlib.$a $(IPPDIR.libia)/libdaalbzlib.$a
daaldep.mac32e.rt  := -L$(TBBDIR.libia) -ltbb $(daaldep.mac32e.rt.$(COMPILER))

daaldep.mac32.mkl.thr := -L$(VMLDIR.libia)/.. -ldaal_mkl_thread
daaldep.mac32.mkl.seq := -L$(VMLDIR.libia)/.. -ldaal_mkl_sequential
daaldep.mac32.mkl := $(VMLDIR.libia)/$(plib)mkl_daal_core.$a
daaldep.mac32.vml := $(VMLDIR.libia)/$(plib)vml_daal.$a
daaldep.mac32.ipp := $(IPPDIR.libia)/libippdaal.$a $(IPPDIR.libia)/libdaalzlib.$a $(IPPDIR.libia)/libdaalbzlib.$a
daaldep.mac32.rt  := -L$(TBBDIR.libia) -ltbb $(daaldep.mac32.rt.$(COMPILER))
...
spat = %.java %.cpp %.h %.txt %.csv %.htm
spat += $(if $(OS_is_win),%.bat %.vcxproj %.filters %.user %.sln,%_$(_OS).lst %makefile_$(_OS) %.sh)
release.SAMPLES.CPP  := $(if $(OS_is_mac),                                                                          \
                          $(filter $(spat),$(shell find $(SAMPLES.srcdir)/cpp -not -wholename '*mpi*' -type f)),    \
                          $(filter $(spat),$(shell find $(SAMPLES.srcdir)/cpp -type f))                             \
                        )
release.SAMPLES.JAVA := $(if $(OS_is_mac),                                                                          \
                        $(filter $(spat),$(shell find $(SAMPLES.srcdir)/java -not -wholename '*hadoop*' -type f)),  \
                        $(if $(OS_is_win),                                                                          \
                        $(filter $(spat),$(shell find $(SAMPLES.srcdir)/java -not -wholename '*spark*' -type f)),   \
...
IFACE.srcdir     := $(DIR)/algorithms/interface
...
IFACE.SERV.srcdir         := $(DIR)/service/interface
...
CORE.objs_a_tpl := $(subst _cpu,_cpu_nrh,$(CORE.objs_a_tmp)) \
...
CORE.objs_y_tpl := $(subst _cpu,_cpu_nrh,$(CORE.objs_y_tmp)) \
...
$(CORE.objs_a): INCLUDES += $(addprefix -I, $(CORE.incdirs))
...
$(CORE.objs_y): INCLUDES += $(addprefix -I, $(CORE.incdirs))
...
$(WORKDIR.lib)/$(thr_tbb_y): LOPT += $(daaldep.rt) $(daaldep.mkl.thr)
...
# Interface part
#===============================================================================
IFACE.srcs     = env_detect.cpp env_detect_features.cpp
IFACE.tmpdir_a = $(WORKDIR)/interface
IFACE.tmpdir_y = $(WORKDIR)/interface_dll
IFACE.objs_a   = $(addprefix $(IFACE.tmpdir_a)/,$(IFACE.srcs:%.cpp=%.$o))
IFACE.objs_y   = $(addprefix $(IFACE.tmpdir_y)/,$(IFACE.srcs:%.cpp=%.$o))
-include $(IFACE.tmpdir_a)/*.d
-include $(IFACE.tmpdir_y)/*.d
...
$(JNI.objs): COPT += $(-fPIC) $(-cxx11) $(-Zl) $(-DEBC) -DDAAL_CHECK_PARAMETER -DDAAL_NOTHROW_EXCEPTIONS
...
$(JNI.tmpdir)/%.res: RCOPT += -D_DAAL_JAVA_INTERF
$(JNI.tmpdir)/%.res: %.rc | $(JNI.tmpdir)/. ; $(RC.COMPILE)

#===============================================================================
# Top level targets
#===============================================================================
daal:    _daal _daal_jj _release _release_doc

daal_dbg:
	@echo $(release.HEADERS.COMMON)
...
#----- releasing static/dynamic TBB/MKL libraries
define .release.t
...
$(foreach t,$(releasemkl.LIBS_Y),$(eval $(call .release.t,$t,$(RELEASEDIR.soia))))
$(foreach t,$(releasemkl.LIBS_A),$(eval $(call .release.t,$t,$(RELEASEDIR.libia))))
